...
// Temps de compilation total: 17.473 ms
//
...
(($t1 = strval(interdire_scripts(((entites_html(sinon(table_valeur(@$Pile[0], (string)'_galerie', null), ''),true)) ?' ' :''))))!=='' ?
		($t1 . (	'
		' .
	recuperer_fond( interdire_scripts(entites_html(table_valeur(@$Pile[0], (string)'_galerie', null),true)) , array_merge($Pile[0],array()), array('ajax' => ($v=( @$Pile[0]['ajax'] ))?$v:true,'compil'=>array('../plugins-dist/medias/formulaires/joindre_document.html','html_d5fac378d7cc98834cd011c693048e3d','',11,$GLOBALS['spip_lang'])), _request('connect')) .
	'
')) :
		'') .
'
